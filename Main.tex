\documentclass{maine-thesis}  % Default options 12pt and final copy

% Include necessary packages here
\usepackage{wlpjbase}
\usepackage{wlpjthm}

% Replace contents of {...} with your own information.
\title{Local Converse Theorem for $2$-dimensional Representations of Weil Groups}					% Title of thesis
\author{William Louis Palmer Johnson}					% Author's name: First Middle Last
\degreesheld{Bachelor of Arts in Mathematics, Williams College 2022}			% Previously earned degree(s), institution(s) and year(s).  
\degree{Master of Arts}  				% Degree to be granted
\program{Mathematics}				% Degree granting department or program
\submitdate{May 2024}				% Month and year of graduation (do not separate with a comma)

\principaladvisor[Dr. Gilbert Moss]{Gilbert Moss, Assistant Professor} 			% Advisor's [short title and name]{name, long title}
% If you have more than one advisor then you'll should delete the first argument ("[...]") above and uncomment the following commands
%\secondadvisor{...}
%\principalshort{...} 			% Shortened advisor name for abstract.  See guidelines for example.

% Include all committee members names and titles
\firstreader{Jack Buttcane, Assistant Professor}        
\secondreader{Tyrone Crisp, Associate Professor}
% If necessary (i.e. for a doctorate), include extra committee members.  Else, comment out or delete any that are unnecessary
%\thirdreader{...}
%\fourthreader{...}
%\fifthreader{...}

% If the thesis has MORE than one appendix, leave the following command in.  Else, comment it out.
%\multipleappendicestrue 

% Begin the document.
\begin{document}
\preliminary
\titlepage

\include{Front}

% Main text of the thesis.  Use of the `\input' command will make later editing much easier.
\include{Ch1}				% Chapter 1
\include{Ch2}				% Chapter 2
\include{Ch3}				% Chapter 3, etc.

% Bibliography files are included here.  Separate with a comma if more than one
%\bibfiles{thesisbibliography.bib}
\chapter{References}
\bibliographystyle{alpha} 		% Appropriate bibliography style
% Appropriate bibliography title, default is REFERENCES, Uncomment to change it.
\renewcommand{\bibname}{}
\bibliography{thesisbibliography}			% Inserts and formats the reference section.

%\appendix					% Include this before any appendices.
%\include{AppA}				% First appendix.
%\include{AppB}				% Second appendix, etc.

\include{Biography}

\end{document}
