...
\begin{theorem}
  ...
  \textcolor{red}{It's Bushnell-Henniart 34.1}
\end{theorem}
...
\begin{proof}
  ...
  $\xi_1 \otimes \xi_1^{-1}$ is the trivial character and is clearly unramified.
  Since $\gamma(\xi_2 \otimes \xi_1^{-1}, X, \psi)$ must have the same poles and zeros as $\gamma(\xi_1 \otimes \xi_1^{-1}, X, \psi)$, we find that $\xi_2 \otimes xi_1^{-1}$ must also be unramified, and agree with $\xi_1\otimes \xi_1^{-1}$ on $\varpi$, making $\xi_1 \otimes \xi_1^{-1} = \xi_2 \otimes \xi_1^{-1}$.
  This is equivalent to $\xi_1 = \xi_2$.
  Combined with $\theta_1 = \theta_2$, we find that $\rho_1 = \rho_2$ as desired.
\end{proof}
...
\begin{proof}
  Let $F$ be a local field with a residue field not of characteristic $2$, and suppose $\rho_1$ and $\rho_2$ are $2$-dimensional tamely ramified semi-simple representations of $\cW_F$, such that for all characters $\chi$ of $\cW_F$, we have
  \[\gamma(\rho_1 \otimes \chi,X,\psi) = \gamma(\rho_2 \otimes \chi,X,\psi),\]
  and $\gamma(\rho_i,X,\psi)$ has $0$ poles.
  ...
  Note that $\gamma(\rho_i \otimes \chi,s,\psi)$ will have at least one pole in this case.

  On the other hand, if $\rho_i$ is a two dimensional irreducible representation of $\cW_F$, then there is some unramified quadratic extension $E_i/F$ and tamely ramified character $\xi_i$ of $\cW_{E_i}$ such that $\rho_i = \ind_{E_i/F} \xi_i$.
  ...
  Since $L(\rho_i) = 1$ for $\rho_i$ irreducible dimension $ > 1$, we have that $\gamma(\rho_i,s,\psi)$ has no poles as expected for this case.
  However, we further have that $\gamma(\rho_i \otimes \chi,s,\psi)$ will never have a pole because $\ind_{E/F} (\xi_i)$ is irreducible if and only if $\xi$ is different than its conjugate, i.e.  $\xi_i \neq \xi_i^{\sigma}$.
  ...
  This allows us to tell apart $\rho_i = \xi_i \oplus \xi_i'$ and $\rho_i = \ind_{E/F} \xi_i$ because the first case will have some twist $\chi$ that gives $\gamma(\rho_i,X,\psi)$ at least one pole and the second case will never have such a $\chi$.

  
  Now, let us deal with the $\rho_i = \xi_i \oplus \xi_i'$ with $\xi_i, \xi_i'$ tamely ramified case.
  This case works out much like the case when $\gamma$ has one pole and $\rho$ is a sum of an unramified and a tamely ramified character.
  First, consider the twist by $\chi = \xi_1^{-1}$.
  ...
  Either we are in the case of Lemma \ref{lem:gamma-one-pole} or Lemma \ref{lem:gamma-two-poles}, but either way, we find that, without loss of generality, $\xi_1 = \xi_2$ and $\xi_1' = \xi_2'$ and so $\rho_1 = \rho_2$.
  ...
  Since the parts in the fraction depend only on the extension $E$ which is fixed, we in fact have that if $\epsilon(\rho_1,X,\psi_F) = \epsilon(\rho_2,X,\psi_F)$, then $\epsilon(\xi_1 ,X,\psi_E) = \epsilon(\xi_2, X,\psi_E)$.
